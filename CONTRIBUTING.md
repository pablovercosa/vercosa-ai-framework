...
O repositório possui CI mínimo em GitHub Actions para pull requests e pushes em `main`. Esse CI valida instalação local em modo desenvolvimento, `pytest`, links Markdown relativos com `python -m vercosa_ai_framework.cli.main docs-links`, executa `python -m vercosa_ai_framework.cli.main alpha-readiness` como diagnóstico não bloqueante e valida `python -m compileall src`; ele não publica pacote, não cria release, não executa missões, não chama providers externos e não usa secrets.
...
Para preparação de release alfa ou revisão conservadora de ambiente novo, use também `PYTHONPATH=src python3 -m vercosa_ai_framework.cli.main alpha-readiness` e o [checklist de instalação limpa](docs/getting-started/clean-install-checklist.md). O comando `alpha-readiness` é diagnóstico auxiliar local e somente leitura; ele não cria tag, não publica release, não publica pacote e não substitui `pytest`, `python3 -m compileall src`, checklist pré-tag ou revisão humana.

O CI mínimo não substitui validação local. Antes de abrir ou revisar um pull request, continue executando `pytest`, `PYTHONPATH=src python3 -m vercosa_ai_framework.cli.main docs-links`, `PYTHONPATH=src python3 -m vercosa_ai_framework.cli.main alpha-readiness`, `python3 -m compileall src` e comandos de CLI seguros quando aplicável, como `PYTHONPATH=src python3 -m vercosa_ai_framework.cli.main validate` e `PYTHONPATH=src python3 -m vercosa_ai_framework.cli.main doctor`.
