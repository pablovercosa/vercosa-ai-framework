...
- A missão 0104 implementou handoff mínimo Mission Runner -> Workflow Engine -> Task Queue por contratos injetáveis. A missão 0108 registrou a separação de responsabilidades como decisão aceita em `docs/architecture/decisions/0001-separar-mission-runner-workflow-engine-task-queue.md`; Mission Orchestrator separado e remoção do caminho legado continuam pendentes.
