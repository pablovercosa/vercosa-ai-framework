...
- Manter persistência local JSONL de eventos auditáveis alinhada a retenção, rotação e relatórios futuros.
...
Status: concluído como persistência JSONL local opt-in em `JsonlAuditEventLog`; retenção, rotação, banco, exportação remota, OpenTelemetry, dashboard e integração automática com scripts continuam futuros.
