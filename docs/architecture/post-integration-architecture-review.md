...
| CLI operacional | Comandos `status`, `missions`, `batch-summary`, `validate`, `doctor`, `docs-links` e `alpha-readiness`. | Não executa missões, Git, testes, scripts, providers, banco ou rede; `docs-links` valida apenas links Markdown relativos locais e `alpha-readiness` diagnostica prontidão alfa sem autorizar release. |
...
- CLI com `status`, `missions`, `batch-summary`, `validate`, `doctor`, `docs-links` e `alpha-readiness`: comandos locais de leitura, listagem de missões por estado, resumo pós-batch auxiliar, validação estrutural, diagnóstico básico, validação local de links Markdown relativos e diagnóstico auxiliar de prontidão alfa.
...
- CLI `status`, `missions`, `batch-summary`, `validate`, `doctor`, `docs-links` e `alpha-readiness`.
