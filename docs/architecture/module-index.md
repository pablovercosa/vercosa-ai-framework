...
| Agentes | `agents/` | [agents](../../src/vercosa_ai_framework/agents/README.md) | MVP | [0008](../../specs/framework/0008-agent-orchestrator.md) | [Agent Orchestrator](../agent-orchestrator.md), [Integração Task/Agent/Capability](task-agent-capability-integration.md), [Governança 0107](execution-governance-0107.md) |
...
- integração orquestrada entre Policy Engine, Context Router, Guardian Engine, Model Selection e Audit/Event Log foi validada no caminho mínimo 0107, mas ainda precisa evoluir para fluxos completos de produção, múltiplos agentes, persistência externa opcional e recuperação governada real;
- fronteira entre Mission Runner e Mission Orchestrator;
- integração global de Policy, Guardian, Context Router, Token Budget, Model Selection e Audit/Event Log ao fluxo Capability -> Skill -> Tool -> Provider Gateway existe no caminho mínimo local 0107 com fakes e dry-run, mas ainda não representa provider real, MCP, rede, banco ou RAG;
