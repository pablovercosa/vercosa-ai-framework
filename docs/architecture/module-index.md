...
- `cli/` oferece consulta local básica de estado operacional, listagem de missões por estado, resumo pós-batch auxiliar, validação estrutural local e diagnóstico local com `doctor`, sem substituir scripts shell, `pytest`, `compileall` nem executar missões nesta fase.
