...
Documentar a relação operacional entre `scripts/vaf-run-next-safe.sh` e `scripts/vaf-run-batch-safe.sh`: execução em batch é o fluxo padrão quando o bloco estiver revisado e seguro; execução individual é o fluxo especial para missões sensíveis, críticas, incertas, investigativas ou de recuperação.
...
A CLI Python inicial em `src/vercosa_ai_framework/cli/` pode consultar status básico local, listar missões por estado, executar validação estrutural local, rodar o diagnóstico auxiliar `doctor` e produzir resumo pós-batch com `batch-summary` por conveniência, mas não substitui este runner, não substitui `scripts/vaf-status.sh`, não substitui `pytest`, não substitui `python3 -m compileall src` e não executa missões nesta fase.

O runner seguro continua sendo o responsável operacional por executar missão. `missions`, `doctor` e `batch-summary` são apenas diagnósticos locais não destrutivos; eles podem ser usados antes ou depois do runner para inspecionar a fila, resumir estado pós-batch e investigar estado estrutural, mas não chamam scripts shell, não iniciam worker e não movem missões.
...
Use `scripts/vaf-run-next-safe.sh` para mudanças arquiteturais profundas, scripts críticos, Guardian Engine, Policy Engine, Context Router, providers, runtimes, missões com dependências incertas, critérios de aceite fracos, recuperação após falha, investigação de erro ou limite de API/quota recém-ocorrido.
...
Depois de um batch, o resumo pós-batch da CLI pode ser executado como apoio de leitura:

```bash
PYTHONPATH=src python3 -m vercosa_ai_framework.cli.main batch-summary
```

Esse comando não executa missões, não verifica worker por processo, não chama scripts shell, não executa Git, não roda `pytest`, não roda `python3 -m compileall src`, não acessa rede e não consulta providers. Ele apenas resume contagens locais, último log encontrado e lembretes de validação manual.
...
O push automático é opt-in. Sem `VAF_AUTO_PUSH=1`, o batch não executa `git push`. Push manual após checklist pós-batch continua sendo a prática recomendada.
...
O batch para na primeira falha. Falhas no runner de uma missão, em `pytest`, em `python3 -m compileall src`, no Git limpo, em `missions/failed`, em `missions/running`, no push final, alteração fora do escopo, dúvida sobre a entrega ou sinal de limite externo interrompem o fluxo.
...
Quando o batch for interrompido por limite externo de API, trate como condição operacional externa, não como bug interno do projeto até prova em contrário. Não insista em retries. Verifique `missions/queue`, `missions/running`, `missions/done` e `missions/failed`; devolva missão presa em `running` para `queue` somente quando for seguro; retome depois que quota, rate limit, crédito ou billing estiverem disponíveis.
...
- O runner individual é adequado para missões sensíveis, críticas, incertas, investigativas, de recuperação ou de alto risco.
- O runner em batch é adequado para blocos locais revisados, seguros e com fila preparada para execução sequencial.
